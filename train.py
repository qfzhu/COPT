...
def train_model(model, disc, env, fields, g_optim, d_optim, data_type, model_opt):
    ...
    trainer = onmt.Trainer(opt, fields, model, disc, env, train_loss, valid_loss, g_optim, d_optim,
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        train_stats = trainer.train(epoch, report_func, opt.training_mode)
        ...
        valid_stats, d_acc = trainer.validate(valid_iter)
        ...
        trainer.epoch_step(valid_stats.ppl(), epoch, opt.training_mode)

        # 5. Drop a checkpoint if needed.
        if epoch >= opt.start_checkpoint_at:
            trainer.drop_checkpoint(model_opt, epoch, fields, valid_stats, d_acc)
...
def load_fields(dataset, data_type, checkpoint):
    ...
    fields['ref'].vocab = fields['tgt'].vocab
...
def build_model(model_opt, opt, fields, checkpoint):
    ...
    model, disc, env = onmt.ModelConstructor.make_base_model(model_opt, fields,
    ...
    return model, disc, env


def build_optim(model, checkpoint, type, optim, learning_rate, reset=True):
    if opt.train_from and checkpoint and not reset:
...
def main():
    ...
    # Build model.
    model, disc, env = build_model(model_opt, opt, fields, checkpoint)
    tally_parameters(model)
    check_save_model_path()

    # Build optimizer.
    if opt.training_mode == 'pre_g' or opt.training_mode == 'pre_d':
        g_optim = build_optim(model, checkpoint, 'g_optim', opt.pre_g_optim, opt.pre_g_learning_rate)
        d_optim = build_optim(disc, checkpoint, 'd_optim', opt.pre_d_optim, opt.pre_d_learning_rate)
    elif opt.training_mode == 'adv':
        g_optim = build_optim(model, checkpoint, 'g_optim', opt.adv_g_optim, opt.adv_g_learning_rate)
        d_optim = build_optim(disc, checkpoint, 'd_optim', opt.adv_d_optim, opt.adv_d_learning_rate)
    else:
        raise

    # Do training.
    train_model(model, disc, env, fields, g_optim, d_optim, data_type, model_opt)
