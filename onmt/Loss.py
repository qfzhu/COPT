...
class LossComputeBase(nn.Module):
    ...
    def monolithic_compute_loss(self, batch, output, attns):
        ...
        rewards = Variable(torch.FloatTensor([[1.0] * batch_size] * (seq_len - 1))).unsqueeze(2)
        if output.is_cuda:
    ...
    def sharded_compute_loss(self, batch, fak_tok, output, fak_output, d1, d2, attns,
...
class NMTLossCompute(LossComputeBase):
    ...
    def sharded_compute_loss(self, batch, fak_tok, output, fak_outputs, d1, d2, attns,
                             cur_trunc, trunc_size, shard_size,
                             normalization, step_type):
        """Compute the forward loss and backpropagate.  Computation is done
        with shards and optionally truncation for memory efficiency.
        ...
        zero_label = Variable(torch.FloatTensor([[0.0]]), requires_grad=False).repeat(batch_size, 1)
        zero_label = zero_label.cuda() if batch.tgt.is_cuda else zero_label
        one_label = Variable(torch.FloatTensor([[1.0]]), requires_grad=False).repeat(batch_size, 1)
        one_label = one_label.cuda() if batch.tgt.is_cuda else one_label

        if step_type == 'd_step':
            d = torch.cat([d1, d2], dim=0)
            label = torch.cat([one_label, zero_label], dim=0)
            loss = self.d_crit(d, label)
            loss.backward()
