    ...
    """ Dataset for data_type=='text'
        ...
        Args:
            ...
            use_filter_pred (bool): use a custom filter predicate to filter
                out examples?
    """
    def __init__(self, fields, src_examples_iter, tgt_examples_iter, ref_examples_iter,
                 num_src_feats=0, num_tgt_feats=0,
                 src_seq_length=0, tgt_seq_length=0, ref_seq_length=0,
        ...
        if tgt_examples_iter is not None:
            examples_iter = (self._join_dicts(src, tgt, ref) for src, tgt, ref in
                             zip(src_examples_iter, tgt_examples_iter, ref_examples_iter))
        else:
        ...
        def filter_pred(example):
            return 0 < len(example.src) <= src_seq_length \
               ...
               and 0 < len(example.ref) <= ref_seq_length
...
class RefField(Field):
    ...
    def process(self, batch, device, train):
        ...
        for i in xrange(ref_len):
            ref = [(' '.join(sample)).strip().split('</s>')[i].strip().split(' ') for sample in batch]
            padded = self.pad(ref)
            ref_tensor = self.numericalize(padded, device=device, train=train)
            ret.append(ref_tensor)
        return ret
