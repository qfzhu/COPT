...
class NMTModel(nn.Module):
    ...
    def infer(self, seq_len, init_state, context, context_lengths, bos_idx, sample=False, temperature=0.5):
        """
        Args:
            prefix: without bos
        """
        ...
        for i in range(seq_len):
            ...
            out = F.softmax(self.generator[0](dec_out) / temperature)
            if sample:
    ...
    def reason(self, seq_len, init_state, context, context_lengths, bos_idx, u, temperature=0.5):
        """
        Args:
            prefix: without bos
        """
        ...
        for i in range(seq_len):
            ...
            out = self.generator[1](self.generator[0](dec_out) / temperature)
            posterior_out = out + u[i]
            _, inp = torch.topk(posterior_out, 1)
